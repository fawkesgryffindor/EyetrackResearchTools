...
string depth_file_name = "C:/Users/varsha/Documents/ResearchEyetrackCode/eyetrack/MyNiViewer/Default/calibrate_headpose_1/DP_For_Katie_9.png";
...
Mat depth_mat = imread(depth_file_name.c_str(), CV_16UC1);
...
//cout<<Mi[x][0]<<","<<Mi[x][1]<<","<<Mi[x][2]<<endl;
	...
	if(g_means.size()>0){
		...
		for(unsigned int i=0;i<g_means.size();++i){
			...
			cout<<mult<<","<<g_means[i][3]<<","<<g_means[i][4]<<","<<g_means[i][5]<<","<<g_means[i][0]<<","<<g_means[i][1]<<","<<g_means[i][2]<<",";
			cout<<head_front[0]<<","<<head_front[1]<<","<<head_front[2]<<","<<head_center[0]<<","<<head_center[1]<<","<<w<<","<<h<<endl;
