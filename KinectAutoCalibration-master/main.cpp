...
// activate to load calibration for one camera and show undistorted images
 #define VIEW_UNDISTORT
	...
	const int corner_cnt_x = 8;
	const int corner_cnt_y = 9;
	const float square_size = 0.016;
//	const char prefix[] = "YOUR_PATH/bin/grab1";
	const char prefix[] = "C:/Users/varsha/Documents/ResearchEyetrackCode/eyetrack/util/calibrate_kinect/Copy_of_calibration5_pairs/pngs";
	const int image_cnt = 18;
	...
	int N = image_cnt*corner_cnt_x*corner_cnt_y;
	...
	for (int i=0; i< image_cnt; i++)
	{
		...
		cout<<"Processing "<<filename_rgb<<"..."<<endl;
...
//				    cvShowImage( "Display window", bw );
//				    cvWaitKey(0);
//		if(cvCheckChessboard(bw, cvSize(corner_cnt_x,corner_cnt_y))==1)
//		{
//			cout<<"Ha ha! found you tricky chessboard"<<endl;
...
//				cvNamedWindow( "Display window", CV_WINDOW_AUTOSIZE );// Create a window for display.
//						    cvShowImage( "Display window", intensity_img );
//						    cvWaitKey(0);
		bw = cvCreateImage(cvGetSize(intensity_img),intensity_img->depth,1);
